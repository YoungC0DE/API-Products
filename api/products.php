...
if ($api == 'produtos') {
    if ($method == 'GET' && $action == 'list') {
        ...
        if ($result) echo json_encode(['dados' => $result]);
        else echo json_encode(['dados' => 'Nenhum produto encontrado!']);
        ...
        if ($resultVerify) {
            ...
            if ($result) echo json_encode(['dados' => 'Produtos adicionados com sucesso!']);
            else echo json_encode(['dados' => 'Erro ao adicionar produtos']);
        }
        ...
        if(!$resultVerify) {echo json_encode(['dados' => 'O produto nao existe']); exit;}

        $query = $db->prepare("DELETE FROM produtos WHERE id = $param");
        $result = $query->execute();

        if ($result)  echo json_encode(['dados' => 'Produto deletado!']);
